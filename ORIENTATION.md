# Orientation — Hailey Emblidge

**Goal:** bring Hailey Emblidge's environment up to the lab standard and get them working in their project repo.

- Project repo: **https://github.com/qc-soule-lab/axial-timeseries**
- Azure container (their own space): **timeseries-hailey**
...
Dr. Soule securely sends them the SAS for **their own** container `timeseries-hailey` (never anyone else's — per-person isolation). Newcomers hit several traps saving it, so use this **exact** method:
...
2. **Create the file with `nano`** — NOT the JupyterLab "Save As" dialog (it can't write hidden dotfolders like `.azure`), and NOT a pasted `read -s … && …` one-liner (a stray newline in the paste breaks it and can echo the secret):
   ```bash
   nano ~/.azure/timeseries-hailey.env
   ```
...
3. **Lock down + verify:**
   ```bash
   chmod 600 ~/.azure/timeseries-hailey.env
   source ~/.azure/timeseries-hailey.env && azure_lake info && azure_lake ls && echo OK
   ```
   `azure_lake info` should show container `timeseries-hailey` + `racwdl` + expiry; `ls` empty + `OK` confirms it reaches Azure. If `info` errors, the SAS got mangled on paste — redo step 2.
